...
export function d7c1(data: string[]): number {
    ...
    while (foundBags.length > 0) {
        ...
        foundBags.forEach((bagWithContent) => {
            const newBags = findBagsWithContent(bagWithContent.name, bagsWithContent);
            newBags.forEach((bag) => {
                if (!indirectly.map(b => b.name).includes(bag.name))
                    indirectly.push(bag);
            });
        });
        ...
        indirectly.forEach((bag) => {
            if (!allBags.map(b => b.name).includes(bag.name)) allBags.push(bag);
        });
    }
...
function findBagByName(name: string): BagContent {
    return bagsWithContent.find((bagWithContent) => bagWithContent.name === name);
}
